...
public extension Inspectable
{
    ...
    public func iterateProperties(_ handler: (_ name: String, _ value: Any, _ idx: Int, _ count: Int) -> Void) {
    ...
    public func deepIterateProperties(_ handler: (_ name: String, _ value: Any, _ idx: Int, _ count: Int) -> Void) {
        let type: Mirror = Mirror(reflecting:self)
        let count = type.children.count
        var idx = 0
        for child in type.children {
